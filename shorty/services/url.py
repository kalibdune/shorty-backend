...
class UrlService:
    ...
    async def create_url(self, url: UrlCreateSchema) -> UrlSchema:
        ...
        await user_service.get_user_by_id(url.user_id)  # check user existing

        url_hash = self.generate_random_hash()
        exisiting_url = await self._repository.get_url_by_hash(url_hash)

        while exisiting_url:
            if exisiting_url.expired_at <= datetime.now():  # TODO do testcase
        ...
        url = UrlInDB(
            url=str(url.url),
            hash=url_hash,
            expired_at=url.expiration_time + datetime.now(),
            user_id=url.user_id,
        )
        print(url)
